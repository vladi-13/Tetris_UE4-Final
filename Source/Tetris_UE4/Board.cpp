...
void ABoard::Tick(float DeltaTime)
{
    ...
    case PS_GOT_BOTTOM:
        ...
        for (auto& Pieza : SiguientesPiezas)
        {
			Pieza->DeletePiece();
		}
        CoolLeft -= DeltaTime;
...
void ABoard::NewPieces()
{
    ...
    CurrentPieces->SpawnBlocks(SiguientesPiezas[0]->GetCoordenadas());
    for (auto& PiecesNexts : SiguientesPiezas) {
        if (PiecesNexts)
        {
            PiecesNexts->Dismiss();
            PiecesNexts->Destroy();
    ...
    float CoordenadasZ = 0.0f;
    for (int i = 2; i >= 1; i--) {
        if (i == 1) {
            SiguientesPiezas[i - 1] = GetWorld()->SpawnActor<APieces>(FVector(0.0f, 125.0f, CoordenadasZ), FRotator::ZeroRotator);
            SiguientesPiezas[i - 1]->SpawnBlocks();
            CoordenadasZ += 30;
        }
        else {
            SiguientesPiezas[i - 1] = GetWorld()->SpawnActor<APieces>(FVector(0.0f, 125.0f, CoordenadasZ), FRotator::ZeroRotator);
            SiguientesPiezas[i - 1]->SpawnBlocks(SiguientesPiezas[i - 2]->GetCoordenadas());
            CoordenadasZ += 30;
        }
    }
...
void ABoard::iniciarPiezas()
{
    ...
    float CoordenadaZ = 185.0f;
    for (int i = 1; i <= 2; i++)
    {
        NextPieces = GetWorld()->SpawnActor<APieces>(FVector(0.0f, 125.0f, CoordenadaZ), FRotator::ZeroRotator);
        CoordenadaZ -= 75.0f;
        SiguientesPiezas.Add(NextPieces);
    }
    for (auto& PiecesNexts : SiguientesPiezas) {
        PiecesNexts->SpawnBlocks();
    }
}